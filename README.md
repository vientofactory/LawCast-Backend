...
이 기능의 목적은 단순히 "무엇이 바뀌었는지"를 보여주는 데서 끝나지 않습니다. LawCast가 원래 갖고 있던 "원문 보존 + 해시 기반 검증 + 사후 증명 가능성"을 유지한 채, 특정 의안번호의 상태 변화와 알림 전송 사실까지 재현 가능한 형태로 남기는 것이 핵심입니다.
...
- 변경 알림은 기존 신규 알림과 분리되어 전송되며, 전송 결과는 `notification_delivery_logs`에 append-only 증적으로 남습니다.
- 프론트 상세 페이지와 변경 API는 같은 체인 데이터를 사용해 리비전 타임라인을 제공합니다.
- ZIP export에는 원문 무결성 정보와 함께 변경 이벤트/전송 증적 스냅샷이 포함됩니다.
...
- 변경 알림 payload는 별도 `payload_hash`를 가지며, webhook 전달 결과와 함께 immutable delivery log에 연결됩니다.
...
- `GET /api/notices/:num/export` 결과물에는 `.changes.json` 파일로 이벤트 타임라인과 delivery log 스냅샷이 함께 포함됩니다.
